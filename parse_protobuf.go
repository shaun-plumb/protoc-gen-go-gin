...
const (
	...
	// phttpPackage   = protogen.GoImportPath("github.com/shaun-plumb/protoc-gen-go-gin/http")
...
// generateHTTPFile generates a _http.pb.go file containing gin handler.
func generateServiceFile(gen *protogen.Plugin, file *protogen.File, gp *GenParam) *protogen.GeneratedFile {
	if len(file.Services) == 0 || (*gp.Omitempty && !hasHTTPRule(file.Services)) {
		return nil
	}
